...
var seckill= {
    ...
    URL: {
        now : function(){
            return  '/test1_war_exploded/seckill/time/now';
        },
        exposer:function(seckillId){
            return '/test1_war_exploded/seckill/'+seckillId+"/exposer";
        },
        execution :function (seckillId,md5) {
            return '/test1_war_exploded/seckill/'+seckillId+'/'+md5+'/execution';
        }
    },
                    ...
                    $('#killBtn').one('click',function(){
                        ...
                        $.post(killUrl,params,function(result){
                            if (result && result['success']){
                                ...
                                //显示秒杀结果
                                if (stateInfo=="秒杀成功"){
                                    node.html('<button class="btn btn-primary btn-lg" id="pay">秒杀成功，立即结算</button>')
                                    $('#pay').one('click',function() {
                                        //alert(seckillId)
                                        $.get("/test1_war_exploded/seckill/seckillPay", {"seckillId":seckillId}, function (result) {

                                            var k= JSON.parse(result);
                                            //alert(result)
                                            var code=k.code;
                                            var total=k.total;
                                            window.location.href="/test1_war_exploded/seckill/pay?code="+code+"&total="+total+"";
    ...
    detail: {
        ...
        init: function (params) {
            //用户手机验证和登陆，计时交互
            //规划我们的交互流程
            //在cookie中查找手机号 !seckill.validatePhone(killPhone)
            ...
            if (false) {
                ...
                $('#killPhoneBtn').click(function () {
                    ...
                    if (seckill.validatePhone(inputPhone)) {
                        //电话写入cookie
                        $.cookie('killPhone', inputPhone, {expires: 7, path: '/test1_war_exploded/seckill'});
                        //刷新界面$.cookie('killPhone', inputPhone, {expires: 7});
